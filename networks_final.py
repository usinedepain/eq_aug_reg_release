...
class eqvConvNet(nn.Module):
    ...
    def __init__(self,omega,lowest_im_dim,out_dim,hidden=(8,8)):
        super().__init__()
        self.conv1 = eqvConv(1,hidden[0],omega)
        self.conv2 = eqvConv(hidden[0],hidden[1],omega)
        self.lin = eqvlinear(hidden[1],lowest_im_dim,out_dim)
...
class omegaConv(nn.Module):
    ...
    def perturb_weights(self,sigma=1e-3):

        with torch.no_grad():
            # save A, then add noise
            noise = sigma*torch.randn(self.calculate_weight_tensor().shape)

            self.weight.data += noise.data.flatten(-2)-self.project_tensor(noise).data.flatten(-2)
           

    def project_tensor(self,weight):

        # calculates projection of a weight onto the equivariant subspace
        proj = weight + torch.rot90(weight,1,[-2,-1]) + torch.rot90(weight,2,[-2,-1]) + torch.rot90(weight,3,[-2,-1])
        proj /= 4
        mask = self.stacked_mask
    ...
    def projection(self):
        ...
        return self.project_tensor(weight)
...
class omegaCNN(nn.Module):
    ...
    def __init__(self,omega,lowest_im_dim=7,out_dim=10,hidden=(8,8),orthogonal=True):
        super().__init__()
        ...
        self.lin = nn.Linear(hidden[1]*lowest_im_dim*lowest_im_dim,out_dim,bias=False)
        self.pool = nn.AvgPool2d(2)
        self.activate = nn.Tanh()
        self.bn1 = nn.LayerNorm(normalized_shape=(14,14),elementwise_affine=False)
        self.bn2 = nn.LayerNorm(normalized_shape=(7,7),elementwise_affine=False)
    ...
    def perturb_weights(self,sigma = 1e-3):
        ...
        # perturbs the linear weights, perpendicular to the equivariant subspace

        with torch.no_grad():
            self.conv1.perturb_weights(sigma)
            self.conv2.perturb_weights(sigma)


            noise= sigma*torch.randn(self.lin.weight.shape)

            self.lin.weight.data = ( self.lin.weight + noise-self.lin_proj(noise)).data



    def lin_proj(self,weight):

        # projects a fully connected layer to the equivariant subspace

        proj = torch.reshape(weight, (self.out_dim,self.hidden[1],self.lowest_im_dim,self.lowest_im_dim))
        proj = proj+torch.rot90(proj,1,[2,3])+torch.rot90(proj,2,[2,3])+torch.rot90(proj,3,[2,3])
        proj /= 4
        proj_lin = torch.reshape(proj, (self.out_dim, self.hidden[1]*self.lowest_im_dim*self.lowest_im_dim))
        return proj_lin
    ...
    def projection(self):
        ...
        proj_lin = self.lin_proj(self.lin.weight)

        return proj_conv1, proj_conv2, proj_lin
    
    def projection_error(self):

        # projects all layers to the equivariant subspace, and returns the error

        pc1,pc2,pl = self.projection()

        return (torch.sum((pc1-self.conv1.calculate_weight_tensor())**2), torch.sum((pc2-self.conv2.calculate_weight_tensor())**2),
                torch.sum((pl-self.lin.weight)**2))
    ...
    def calculate_weight_tensor(self):
        ...
        return weight_conv1, weight_conv2, weight_lin
...
def compare_nets(net,enet):
    c1e = ((net.conv1.calculate_weight_tensor()-enet.conv1.calculate_weight_tensor())**2).sum()
    c2e = ((net.conv2.calculate_weight_tensor()-enet.conv2.calculate_weight_tensor())**2).sum()
    le = ((net.lin.weight-enet.lin.calculate_weight_tensor())**2).sum()
